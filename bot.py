...
Bot Made With ❤️ By @mycoursefreee**
""" 
...
#acorn 
@warnerstarkbot.on(events.NewMessage(pattern="^/(AcornTv)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, warnerstarkbot)
    	if starky is False:
...
#voot
@warnerstarkbot.on(events.NewMessage(pattern="^/(voot)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, warnerstarkbot)
    	if starky is False:
...
@warnerstarkbot.on(events.NewMessage(pattern="^/(zee5)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, warnerstarkbot)
    	if starky is False:
...
@warnerstarkbot.on(events.NewMessage(pattern="^/(cfd)$"))
async def hmm(event):
    if Config.JTU_ENABLE:
    	starky = await check_if_subbed(Config.CHANNEL_USERNAME, event, warnerstarkbot)
    	if starky is False:
